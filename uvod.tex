\chap Úvod
%citace, spravující nebo ovládací
...
Mnoho subsystémů uchovává informace o~struktuře device a~tyto informace využívá k~dalším úkonům.
...
*Statistiky -- například statistiky jako počet odeslaných paketů a~paketů přiajtých.
*Stav zařízení
*Správa přenosu -- nastavení kvality služeb (QoS).
*Tabulka virtuálních funkcí -- je tabulku ukazatelů implementovaných ovladačů zařízení. Obsahují funkce jako je~aktivace portů, deaktivování portů, nastavení adres a~mnohé další popsané v~dokumentaci Linuxového jádra nebo \cite[ULNI].
\enditems
...
Ovladač v~periferních zařízeních, neboli \uv{USB gadget drivers}, které kontrolují připojená zařízení přes USB.
...
Tato struktura obsahuje informace o~reálné informace koncovém bodu připojeného zařízení ve~struktuře {\em usb\_endpoint\_descriptor}.
...
\caption/f Hierarchie USB poposuje jak jsou zaobaleny jednotlivé části USB subsystému. Obrázek převzat z~\cite[LDD3]. 
%TODO XXX popisek
\medskip
...
Příklad cesty v sysfs reprezentující k strukturu {\em usb\_device} může vypadat následovně:
...
Obsahuje tabulkou virtuálních funkcí a~umožňuje přístup k~registrům popsaných v~sekci \ref[miireg].
...
Konfigurace přepínače, jako je~nastavení portů a~správu MII rozhraní, se děje na~základě struktury okazatelů na funkce {\em ethtool\_ops}, kterou využívá například nástroj Ethtool \fnote{Ethtool je~standardní nástroj Linuxu pro~ovládání a~podporu ovladačů a~zařízení \cite[EMBLINUX].}.
...
\caption/f Diagram tříd vyhraných datových struktur DSA subsystému.
\medskip
...
Instance subsystém DSA je~reprezentován strukturou {\em dsa\_switch\_tree}, která obsahuje odkazy na~struktury:
\begitems
...
Většina atributů je~reprezentována textovými soubory, které jsou~v~kódování ASCII a~obsahuje pouze jednu hodnotu.
...
Sysfs je~reprezentováno v~Linuxovém jádře jako struktura {\em kerfs_root}, která se skládá z~uzlů {\em kernfs\_node}.
Tyto struktury využívají instance struktur kobj k reprezentaci dat a také obsahují tři druhy obslužného volání -- na~zápis, čtení a~na uvolňení.
...
Má velké množství komunikačních rozhraní jako je~I2C, SPI, UART, MMC, USB a Ethernet a~výstupem pro PWM.
...
\caption/f Procesorový modulu Colibri T20 \cite[SOC].
\medskip
...
\caption/f Bloky procesoru NVIDIA Tegra 2 \cite[TEGRA].
\medskip
...
AX88772b má modifikovatelnou vícefunkční sběrnici, která umožňuje připojit RMII, popsané v sekci\ref[RMIIsub], nebo Reverse-RMII způsobem MAC--to--MAC k~mikrokontroléru s~Ethernet MAC RMII rozhraním.
...
Jak lze videt na~obrázku \ref[MARVELL6060], který popisuje obvod Marvell 88E6060, který je~velmi podobný obvodu Marvell 88E6065.
...
Tento kontrolér je~připojen přes RMII rozhraní k~čipu Marvell 88E6065 \ref[MARVELL] tak, viz na~obrázku níže \ref[MARVELL6065rmii].
...
Dále bylo potřeba stáhnout nástroj, které by sloužily k~vytvoření cílového binárního souboru zavaděče U-Boot.
...
\fnote{Kernel Memory Leak Detektor je~nástroj v~Linuxovém jadře, která umožňuje nalezení špatného uvolňení paměti. Funguje podobně jako trasující garbage collector.}.
...
Nastavení bylo provedeno Linuxovým nástrojem ifconfig a~ip.
