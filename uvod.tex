...
Media Independent Interface je typ rozhraní, který umožňuje připojení nezávyslého na procesoru a k fyzickém médiu.
Media Independent Interface (MII) je specifikováno standardem IEEE 802.3 v kapitole 22 \cite[IEEE8023].
MII propojuje dvě vrstvy ISO/OSI a to vrstvu Data Link, konkrétně její část Media Access Constroll (MAC)
...
a vrstvou fyzickou (PHY)
\fnote{PHY převádí bitový stream na signál a definuje jeho doručení.}.
MII rozhraní podporuje dva datové toky a těmi jsou 10~Mb/s a 100~Mb/s.
Rozhraní Gigabite Media Independent Interface podporuje rychlost až 1~Gb/s.
Funkcionalita MII je identicka na obou datových tokách a liší se pouze v nominální frekvenci hodin.
...
Na obrázku \ref[MIItoISO] můžeme vidět připojení MII na Reconsiliation Sublayer (RS) a PCS nebo PLS.
...
Konkrétní mapování MII signálů můžeme vidět na obrázku \ref[RStoMII].

\medskip
\clabel[RStoMII]{Mapování signálů RS to MII}
\picw=15cm \cinspic images/802_rs.pdf
\caption/f Obrázek ukazuje jak RS mapuje signály mezi MII a PLS. Take je zde vidět připojení STA \cite[IEEE8023].
\medskip
...
K STA může být připojeno až 32 fyzickým jednotkám. které může být obsluhováno jedním menežovacim rozhraním STA.

\label[MIIdat]

\secc Datová část MII rozhraní

Datová část MII disponuje několika druhy signálu, které mužeme vidět na pravé straně obrázku \ref[RStoMII], které jsou připojeny k přemapovací vrstvě RS.
Druhy datových signálů jsou popsany níže:
...
Na obrázku \ref[transiotionMII] můžeme vidět jak může vypadat komunikace po MII rozhraní.
TX\_CLK je referenční hodinový signál, který se pravidelně mění.
Při odesílání dat se nahodí signál TX\_EN a začne odesílat preambuly, tak jak je to popsáno níže.
A jelikož začne být příjemce zaneprázněn nahodí signál CRS.
Tato komunikace je pouze jedním směrem a též neobsahuje žádnou chybu, jak je vidět ze signálu COL.

Rámec dat, který je přenášen po MII rozhraní je doplněn o hlaviky a patičky popsáné formátem:
...
Rámce jsou přenášeny tak, že oktet je rozdělen na dvě poloviny tzv. nibble. A ty jsou odeslány ve dvou částech po datových linkách TXD nebo RXD.
Příklad odeslání oktetu po pinech TDX jak můžeme vidět na obrázku níže \ref[NIBBLE].
...
Každé zařízní na té nejnižší úrovni je v operačním systému Linux je reprezentováno instancí struktory {\em device}.
...
Je velmi optížné dnes narazit na zařízení, které je reprezentováno pouze struktorou {\em device}.
Místo toho  některé struktury v struktuře {\em device}, jako je kobj, obecně implementují vyšše reprezentovaná zařízení \ref[DEV].
\fnote{Popis datové struktury {\em device} můžete najít na \url{http://lxr.free-electrons.com/source/include/linux/device.h\#L730}}.
...
Na obrázku níže můžeme vidět diagram tříd důležitých struktur v subsystému DSA, které jsou důležíté pro další vývoj \ref[umlDSA].
...
Na tomto obrázku jsou zvírazněny struktury {\em mii\_bus}, {\em net\_device} a {\em dsa\_platform\_data}.
Tyto struktury DSA subsystém nevytváří a je potřeba tyto struktury předat.

DSA je reprezentována strukturou {\em dsa\_switch\_tree}, která obsahuje reference na struktury
\begitems
*dsa\_platform\_data -- tato struktura informuje o tom jaká zařízení jsou připojená ke kořenovému přepínači. Obsahuju struktur {\em dsa\_chip\_data} popsanou níže.
*net\_device -- reference na síťové zařízení \ref[NETDEV], reprezentující hlavní síťové zařízení.
*dsa\_switch -- data pro jednotlivé přepínače.
\enditems

Struktura {\em dsa\_switch} nese data o připojeném přepínači.
\begitems
*dsa\_switch\_tree -- reference na rodičovskou strukturu.
*dsa\_chip\_data -- Tato struktura obsahuje informace o fyzickém přepínači -- kolik má portů a jaké jsou jejich názvy. Též obsahuje informace o DT.
*device -- reference na hostovské zařízení, které se využívá.
*dsa\_switch\_driver -- struktura popisující ovladač přepínaču, obsahuje funkce na menežování připojeného přepínače.
*mii\_bus -- ovladač MII sběrnice.
*net\_device -- reference na síťové zařízení \ref[NETDEV], reprezentující port zařízení.
\enditems
...
Struktura {\em net\_device } je struktura popisující síťové rozhraní \cite[ULNI].
Síťové rozhraní může příjímat a vysílat velké množství dat a proto je implementace podobná diskovým zařízením \cite[LDD3].
...
Tato struktora obsahuje informace o realném koncovém bodu v struktuře {\em usb\_endpoint\_descriptor}.
Struktura {\em usb\_endpoint\_descriptor} popisuje data pomocí příjmutých dekriptorů.

\medskip
\clabel[USBMAP]{Za.}
\picw=15cm \cinspic images/usbendpmap.pdf
\caption/f Na obrázku Obrázek převzat z \cite[LDD3].
\medskip
...
USB repoduktory mohou mít dvě rozhraní jako je USB klávesnice pro tlačítka a USB audio stream \cite[LDD3].%TODO presunout nahoru
...
A však Linuxové jádro nemůže obsluhovat více konfigurací v jeden čas. Linuxové jádro popisuje konfigurace v struktuře {\em usb\_host\_config} a celé USB zařízení strukturou {\em usb\_device}.
Ovladače USB zařízení  obyčejně přepisují data z {\em usb\_interaface} do {\em usb\_device}.

Samotné struktury {\em usb\_device} a {\em usb\_interface} jsou zobrazeny v sysfs\ref[SYSFS] jako jednotlivá soubory.

Na řádce níže můžeme vidět ukázkovou cestu k struktuře {\em usb\_device} reprezentovanou pomocí sysfs:

\begtt  /sys/devices/pci0000:00/0000:00:09.0/usb2/2-1  \endtt

K výše uvedené cestě k USB zařízení je na řádku níže zvírazněno USB rozhraní reprezentováno strukturou {\em usb\_interface} je pojmenovan podle formátu root\_hub-hub\_port:config.interface (pro hlubší stromy je schéma root\_hub-hub\_port-hub\_port:config.interface):
...
Na obrázku níže  můžeme vidět uml diagram důležitých struktur v ovladači AX88772b \ref[umlASIX].
...
Jam můžeme vidět na obrázku, usbnet obsahuje informace o :
\begitems
*net\_device -- zařízení reprezentující síťové rozhraní
*driver\_info -- rozhraní pro ovladač usbnet zařízeních
...
Sysfs je reprezentováno v Linuxovém jádře jako struktura {\em kobj}, který obsahuje tři druhu obslužného volání na zápis, čtení a na uvolňení.
