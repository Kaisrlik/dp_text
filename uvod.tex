...
Frekvence tohoto signálu by měla být 20\% z nominální hodnoty přenosu dat, tedy +- 100ppm. %TODO plus minus
...
Místo toho  některé instance ve struktuře {\em device}, jako je kobj, obecně implementují výše reprezentovaná zařízení \ref[DEV]. % TODO XXX divny kobj moc brzo XXX
...
Jednotlivé složky struktury {\em net\_device } mohou být klasifikovány do několika kategorií:
%TODO divne nejsou v DSA implementovany jako vitualni porty??? XXX
\begitems
...
Nastavování přepínače, jako je nastavení portů a menežování MII rozhraní, je provedeno na základě rozhraní ethtool \fnote{Ethtool je standardní utilita Linuxu pro ovládání a podporu ovladačů a zařízení.}.
...
Na tomto obrázku jsou zvýrazněny struktury {\em mii\_bus}, {\em net\_device} a {\em dsa\_platform\_data}. %TODO opravit obrazek zviraznit dsa platform data
...
Colibri T20, vyráběný firmou Toradex \ref[TODADEX], je počítačový modul postaven na základě NVIDIA Tegra 2 vestavěném systému na čipu \cite[SOC].
...
Jelikož se jedná o systém na čipu, obsahuje tento čip mnoho bloků, jako je například Audio/Video rekordér, grafický procesor s podporou 2D rendrovaní a 3D pixel a vektor shadrem. %cesky ekvivalent by byl nejlepsi XXX
...
Jako jediný z vyráběných modelů dosahuje v revizi AX88772bli teplotních rozsahů od -40 stupňů Celsia až +85 stupňů Celsia. Což odpovídá industriálním teplotním rozsahům.
%x^{\circ} XXX
...
\fnote{QoS je kontrolní mechanismus, který garantuje přenosovou kapacitu.}. %TODO XXX
...
Konfigurovatelný přepínač umožňuje nastavení ethernetového přepínače za pomoci USB Ethernet kontroléru AX88772b od firmy Asix \ref[ASIX], ke kterému je připojena externí pamět EEPROM M93-C66 \fnote{ Odkaz na pamet TODO XXX}.
...
\fnote{Co je RC TODO XXX}.
Deska tak měla během této doby možnost, aby se vstupní hodnoty na všech pinech ustálily ve správném stavu.

Pro zajištění správného a deterministického chování přepínače byly přidány pull-up a pull-down rezistory.
\begitems
%TODO XXX
\enditems
...
Uživatel Andrew Lunn se vyjádřil k mému nápadu a navrhl několik optimalizací. %TODO XXX reepsat optimalizace
...
64 bytes from 192.168.202.222: icmp_seq=2 ttl=64 time=0.427 ms
64 bytes from 192.168.202.222: icmp_seq=3 ttl=64 time=0.466 ms
64 bytes from 192.168.202.222: icmp_seq=4 ttl=64 time=0.462 ms
64 bytes from 192.168.202.222: icmp_seq=5 ttl=64 time=0.432 ms
64 bytes from 192.168.202.222: icmp_seq=6 ttl=64 time=0.393 ms
64 bytes from 192.168.202.222: icmp_seq=7 ttl=64 time=0.466 ms
64 bytes from 192.168.202.222: icmp_seq=8 ttl=64 time=0.410 ms
64 bytes from 192.168.202.222: icmp_seq=10 ttl=64 time=0.399 ms
^C
...
kajza ~/dp/main_linux/linux_build $ scp ~/Downloads/The.Avengers.2012.1080p.BR.2GB.ShAaNiG.com.mkv aolad@192.168.202.222:~/
aolad@192.168.202.222's password: 
The.Avengers.2012.1080p.BR.2GB.ShAaNiG.com.mkv                                                       100% 2044MB   9.9MB/s   03:26    
kajza ~/dp/main_linux/linux_build $ dd if=/dev/zero bs=16000 count=625 | nc -v 192.168.202.222 2112
...
^CExiting.
kajza ~/dp/main_linux/linux_build $ dd if=/dev/zero bs=160000 count=625 | nc -v 192.168.202.222 2112
192.168.202.222 2112 (idonix-metanet): Connection refused
kajza ~/dp/main_linux/linux_build $ dd if=/dev/zero bs=160000 count=625 | nc -v 192.168.202.222 2112
192.168.202.222 2112 (idonix-metanet) open
625+0 records in
625+0 records out
...
^CExiting.
kajza ~/dp/main_linux/linux_build $ dd if=/dev/zero bs=1600000 count=625 | nc -v 192.168.202.222 2112
192.168.202.222 2112 (idonix-metanet) open
625+0 records in
625+0 records out
1000000000 bytes (1.0 GB) copied, 85.9629 s, 11.6 MB/s
^CExiting.
kajza ~/dp/main_linux/linux_build $ dd if=/dev/zero bs=160000 count=6250 | nc -v 192.168.202.222 2112
192.168.202.222 2112 (idonix-metanet) open
6250+0 records in
6250+0 records out
1000000000 bytes (1.0 GB) copied, 85.9921 s, 11.6 MB/s
kajza ~/dp/main_linux/linux_build $ dd if=/dev/zero bs=16000 count=625 | nc -v 192.168.202.222 2112
192.168.202.222 2112 (idonix-metanet) open
625+0 records in
625+0 records out
10000000 bytes (10 MB) copied, 0.874124 s, 11.4 MB/s
kajza ~/dp/main_linux/linux_build $ 
