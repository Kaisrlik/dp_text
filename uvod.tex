...
Místo toho některé instance ve struktuře {\em device}, jako je kobj, obecně implementují zařízení, která jsou nadřazená struktuře {\em device} \ref[DEV].
...
Nastavování přepínače, jako je nastavení portů a menežování MII rozhraní, je provedeno na základě virtuální tabulky funkcí {\em ethtool\_ops}, kterou využívá například nástroj Ethtool \fnote{Ethtool je standardní utilita Linuxu pro ovládání a podporu ovladačů a zařízení \ref[EMBLINUX].}.
...
Na jednom ze dvouch počítačů zapnememe server, který bude poslouchat na portu např. 4242:

\begtt
$nc -lk -z 4242 > /dev/null
\endtt

A na druhém z počítačů zapneme přenos dat na druhý počítač s danou IP adresou a daným portem:
A budeme sledovat jak se data liší na různých velikostech dat.
...
Tyto testy také byla prováděny na různých velikostech a rychlost přenosu se pohybovala mezi 11.3~MB/s až 12.1~MB/s
