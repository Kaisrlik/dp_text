...
%TODO divne nejsou v DSA implementovany jako vitualni porty???
\begitems
...
V průběhu tohoto testování bylo objeveno několik chyb v návrhu desky. která byla na základě mých připomínek opravena.
...
Toto zařízaní inicializuje daný ovladač do standardního stavu na základě funkce usb\_probe(), která podle ID produktu a výrobce vybere správný ovladač.
...
Všechny tyto instance nebo jejich odkazy by měla předat struktura platform\_device. Zmíněné struktury lze vidět v \ref[umlDSA].

Za pomoci těchto dat by se v DSA subsystému měl vytvořit ovladač pro přepínač.
Ovladač by si pak měl najít potřebné funkce pro konkrétní přepínač specifikované strukturou dsa\_switch\_driver a pro každý port by měl vytvořit nové vlastní síťové rozhraní.
...
Při odpojení desky od modulu by mělo toto uvolnění poskytovat USB ovladač Asix AX88772b.
...
To však nemůže být aktivní současně s rozhraním ethernet PHY, které je taktéž integrováno na čipu, jak je zobrazeno \ref[ASIXRMII].
...
Protože toto zařízení nebylo schopno mít aktivováno více komunikačních rozhraní najednou, bylo potřeba doplnit ovladač o aktivaci RMII rozhraní. 
...
Další funkcí dané rutiny bylo testování, zda bylo k USB kontroléru připojeno zařízení Marvell 88E6065 dle standardizováného registrového prostoru \ref[miireg], konkrétně vyčtením z třetího registru z \ref[miireg], který obsahuje ID a revizi zařízení.
...
Po naplnění základních struktur ovladač Asix AX88772b zavolal funkci {\em dsa\_probe\_netdevice()}, která tvořila softwarové rozhraní přepínače. Změna DSA subsystému je popsaná v \ref[dsadri].
...
Z důvodu problému v komunikaci bylo potřeba odloučit datovou strukturu DSA od platform\_driveru, což následně způsobilo rozšíření dsa\_switch\_driver \ref[mvdri].

V uvolňovací části ovladače bylo potřeba provést několik změn, jelikož v původním provedení se nepředpokládalo, že by se tento ovladač někdy uvolňoval. To plyne z debaty na \cite[ML].
Funkce na uvo'nování ovladače nebyla v původní verzi dokončena, a proto byla teď upravena část uvolňovací rutiny {\em dsa\_destroy()}, která v současné verzi podporuje uvolnění všech portů interpretovaných strukturou net\_device a interních struktur DSA ovladače, jako je struktura {\em dsa\_switch a dsa\_switch\_tree}.
...
Tento subsystém je schopen vytvořit dostatek ethernetových rozhraní a připojit příslušný ovladač pro přepínací obvod.
